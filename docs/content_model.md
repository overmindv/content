# Bumblebee content model

`bumblebee` хранит самостоятельные контентные документы и их версии.

Сервис не владеет каталогом, пользователями, ролями, заданиями, обсуждениями и review workflow. Другие сервисы могут ссылаться на контент через `content_item.id`, но `bumblebee` не хранит связи с вузами, программами, курсами или темами.
...
`created_by` и `updated_by` являются opaque user id. `bumblebee` не знает модель пользователя и не проверяет роли.
...
Теги нужны для базовой группировки и поиска по контенту внутри `bumblebee`. Если позже появится глобальный сервис таксономии, `tags` можно будет заменить или синхронизировать с ним.
...
`asset_id` является внешним id файла из media/file сервиса. `bumblebee` не хранит бинарные файлы.
...
- `slug` уникален внутри `bumblebee`.
